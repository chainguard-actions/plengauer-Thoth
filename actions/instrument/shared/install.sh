...
if ! type otel.sh 2> /dev/null; then
  ...
  if [ "$GITHUB_REPOSITORY" = "$GITHUB_ACTION_REPOSITORY" ] && [ -f "$GITHUB_WORKSPACE"/opentelemetry-shell_"$version"_"$arch".deb ]; then
    ...
    sudo -E -H apt-get -o Binary::apt::APT::Keep-Downloaded-Packages=true install -y "$debian_file"
      ...
      rm /tmp/package.deb
    ) || ( echo ::warning::'Cannot find any release in repository, falling back to root repository' >&2 && INSTALL_SCRIPT="$(mktemp)" && wget -O "$INSTALL_SCRIPT" https://raw.githubusercontent.com/plengauer/Thoth/main/INSTALL.sh && sh -e "$INSTALL_SCRIPT"; rm -f "$INSTALL_SCRIPT" )
fi
